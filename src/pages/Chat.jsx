...
const Chat = () => {
    ...
    const fetchContacts = async () => {
        ...
        if (res.success) {
            // SMART SORTING: Newest message first
                        ...
                        {filteredContacts.length > 0 ? (
                            filteredContacts.map(contact => (
                                <div
                                    ...
                                    className={clsx(
                                        "flex items-center gap-4 p-4 cursor-pointer transition-all duration-200 border-b border-slate-50 hover:bg-slate-50",
                                ...
                                >
                                    ...
                                    <div className="relative">
                                        <div className={clsx(
                                            ...
                                            activeChat?.id === contact.id ? "bg-blue-500 text-white" : "bg-slate-100 text-slate-500"
                                        )}>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-baseline mb-1">
                                            <h3 className={clsx(
                                                "text-sm truncate",
                                                activeChat?.id === contact.id ? "font-bold text-slate-800" : "font-semibold text-slate-700"
                                            )}>
                                                {formatName(contact)}
                                            </h3>
                                        ...
                                        <div className="flex justify-between items-center">
                                            <p className={clsx(
                                                ...
                                                contact.unreadCount > 0 ? "font-semibold text-slate-600" : "text-slate-400"
                                            )}>
                                                {contact.lastMessage || "Mulai percakapan baru"}
                                            </p>
                                            {contact.unreadCount > 0 && (
                                                <div className="w-5 h-5 bg-red-500 rounded-full flex items-center justify-center text-[10px] font-bold text-white shadow-sm shadow-red-200">
                                                    {contact.unreadCount}
                                                </div>
                                            )}
                                        </div>
                        ...
                        <>
                            ...
                            <div className="h-20 bg-white border-b border-slate-100 flex items-center justify-between px-6 shadow-sm z-10">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-slate-800 text-lg">{formatName(activeChat)}</h3>
                                            ...
                                            {isUserOnline(activeChat.last_active_at) ? (
                                                ...
                                                <p className="text-xs text-slate-400 font-medium">
                                                    {activeChat.last_active_at
                                                        ? `Terakhir dilihat ${new Date(activeChat.last_active_at).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}`
                                                        : 'Offline'}
                                                </p>
                                            )}
                                        </div>
                                    ...
                                    return (
                                        <React.Fragment key={idx}>
                                            ...
                                            <div key={idx} className={clsx("flex w-full", isMe ? "justify-end" : "justify-start")}>
                                                <div className={clsx(
                                                    "max-w-[70%] md:max-w-[60%] rounded-2xl p-4 shadow-sm relative group transition-all",
                                                    isMe
                                ...
                                <form onSubmit={handleSendMessage} className="flex items-center gap-3 max-w-4xl mx-auto">
                                    ...
                                    <div className="flex-1 relative">
                                        <input
                                            ...
                                            className="w-full py-3.5 pl-5 pr-12 bg-slate-50 border-none rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:bg-white transition-all shadow-inner"
                                        />
                                    ...
                                    <button
                                        ...
                                        className="p-3.5 bg-blue-600 text-white rounded-full hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-blue-500/30 transition-all transform hover:scale-105 active:scale-95"
                                    >
